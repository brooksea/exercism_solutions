...
export function scaleRecipe(recipe, scaler) {
  const scaledRecipe = {}
  const recipeArr = Object.entries(recipe).map(([ingredient, amount]) => [ingredient, amount * scaler/2])
  recipeArr.forEach(([ingredient, amount]) => scaledRecipe[ingredient] = amount)
  return scaledRecipe
}